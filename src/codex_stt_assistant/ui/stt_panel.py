...
class STTPanel(Gtk.Box):
    ...
    def _build_layout(self):
        ...
        self._left_split_applied = False
        ...
        def _apply_split():
            ...
            min_bottom = 240
            pos = int(height * 0.6)
            pos = max(min_top, min(pos, height - min_bottom))
            self.paned.set_position(pos)
            self._left_split_applied = True
            return False

        GLib.timeout_add(100, _apply_split)
    ...
    def _build_chat_panel(self):
        ...
        self.text_view.set_size_request(-1, 120)

        self.text_buffer = self.text_view.get_buffer()
        self.text_buffer.connect("changed", self._on_buffer_changed)

        self.placeholder = Gtk.Label(label="Type a message… (Ctrl+Space to record)")
        self.placeholder.set_xalign(0.0)
        self.placeholder.set_margin_top(6)
        self.placeholder.set_margin_start(6)
        ...
        controls.add_css_class("composer-row")
        ...
        controls.append(mic_box)
        controls.append(self.record_button)
        controls.append(self.stop_button)
        controls.append(self.level_bar)
        controls.append(self.auto_submit)
        controls.append(spacer)
        controls.append(self.clear_button)
        controls.append(self.char_count)
        ...
        self.composer.append(frame)
        self.composer.append(controls)
