...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def _build_statusbar(self):
        ...
        # Use a vertical box instead of overlay to prevent blocking content
        main_layout = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self._paned.set_vexpand(True)
        main_layout.append(self._paned)
        main_layout.append(self.status_bar)
    ...
    def _on_stt_partial(self, text):
        # val = self.stt_panel.is_live_mode()
        # print(f"DEBUG: MainWin Partial: '{text}' Live: {val}")
        GLib.idle_add(self.stt_panel.set_partial, text)
        if self.stt_panel.is_live_mode() and text.strip():
            # Send LIVE typing to terminal (Clear Line + Text)
            # using \x15 (Ctrl+U) to clear input line and retype
            # We use idle_add to ensure thread safety
            GLib.idle_add(self.terminal_panel.send_partial, f"\x15{text}")
