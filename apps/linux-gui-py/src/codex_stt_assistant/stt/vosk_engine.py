...
class VoskEngine(STTEngine):
    def __init__(self, config, on_result=None, on_partial=None, on_status=None, on_level=None):
        ...
        # Use smaller chunk size for better responsiveness (override default 4096)
        cfg_chunk = int(audio_config.get('chunk_size', 4096))
        self.chunk_size = 1024 if cfg_chunk >= 4096 else cfg_chunk
        self.device_index = audio_config.get('device_index')
        print(f"DEBUG: VoskEngine init. Chunk size: {self.chunk_size}")
        ...
        self._silence_duration = float(stt_config.get('auto_stop_silence_duration', 1.0))
        self._silence_threshold = 0.02 # Adjust based on noise floor
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Enable partial words for better real-time feedback
    ...
    def _process_loop(self):
        while not self._stop_event.is_set():
            ...
            # Manual Silence Detection logic
            ...
            # Check for forced result due to silence
            silence_duration = time.time() - self._last_speech_ts
            force_result = self._has_partial and (silence_duration > self._silence_duration)

            if force_result:
                # Force final result from accumulated partials
                # Note: Result() resets the decoder, effectively splitting the sentence
                ...
                self._has_partial = False
                self._last_speech_ts = time.time() # Reset wait
            elif self._recognizer.AcceptWaveform(audio_data):
                ...
                if partial_text:
                    ...
                    print(f"DEBUG: Vosk Partial: {partial_text}")
                    self.emit_partial(partial_text)
                else:
                     # If partial is empty but we had content before, keep flag? 
                     # Usually Vosk clears partial if it's not confident.
                     pass

    def _calculate_rms(self, item):
        try:
            # Handle tuple from AudioCapture
